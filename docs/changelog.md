...
- Removed the `title_fn` in `mangdl/api/providers/templates/wordpress.py`'s `template.manga` function due to the said variable not being used anywhere.
- Fixed `mangdl/api/providers/firstkiss.py`'s `manga_id_fn` function to receive `url` and keyword arguments instead of `soup`.
